 ...
 */
uint32_t arm_uc_pal_flashiap_round_up_to_page_size(uint32_t size)
{
 ...
 */
arm_uc_error_t arm_uc_pal_flashiap_get_slot_addr_size(uint32_t slot_id,
                                                             uint32_t *slot_addr,
                                                             uint32_t *slot_size)
{
                                         ...
                                         const arm_uc_buffer_t *buffer)
{
    ...
    if (buffer && buffer->ptr && result.error == ERR_NONE) {
        ...
        if ((write_size % page_size == 0) && (physical_address % page_size == 0)) {
            UC_PAAL_TRACE("programming addr %" PRIX32 " size %" PRIX32 " page %u",
                          physical_address, write_size, page_size);
