...
arm_uc_error_t ARM_UC_cryptoHashSetup(arm_uc_mdHandle_t *hDigest, arm_uc_mdType_t mdType)
{
    ...
    if (hDigest) {
        ...
        if (mbedtls_md_setup( hDigest, mbedtls_md_info_from_type(mdType), 0 ) == 0 &&
            mbedtls_md_starts(hDigest) == 0) {
...
arm_uc_error_t ARM_UC_cryptoHashFinish(arm_uc_mdHandle_t *hDigest, arm_uc_buffer_t *output)
{
    ...
    if (hDigest && output && (output->size_max >= (unsigned)mbedtls_md_get_size(hDigest->md_info))) {
#else
    if (hDigest && output && (output->size_max >= (unsigned)mbedtls_md_get_size(mbedtls_md_get_handle(hDigest)))) {
#endif /* !defined(MBEDTLS_MD_SINGLE_HASH) */
        int mbedtls_result = mbedtls_md_finish(hDigest, output->ptr);

        if (mbedtls_result == 0) {
            result = (arm_uc_error_t) { ERR_NONE };
#if !defined(MBEDTLS_MD_SINGLE_HASH)
            output->size = mbedtls_md_get_size(hDigest->md_info);
#else
            output->size = mbedtls_md_get_size(mbedtls_md_get_handle(hDigest));
#endif /* !defined(MBEDTLS_MD_SINGLE_HASH) */
        }
    }
