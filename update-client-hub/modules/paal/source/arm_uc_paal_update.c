...
#if defined(ARM_UC_FEATURE_DELTA_PAAL) && (ARM_UC_FEATURE_DELTA_PAAL == 1)
    paal_update_implementation = &ARM_UCP_DELTA_PAAL;  // todo note this forces delta paal even for normal updates
...
#ifdef TRACE_WRITES
    #define PRINTAMOUNT 50
    #define MIN(x,y) (((x)<(y)) ? (x) : (y))
    int size = buffer->size;
    for(int j=0;j<size; j+=PRINTAMOUNT)
    {
        int amount = MIN(size-j, PRINTAMOUNT);
        UC_PAAL_TRACE("W(%d-%d): %s",j,j+amount, mbed_trace_array(buffer->ptr+j, amount+1 ));
    }
#endif
