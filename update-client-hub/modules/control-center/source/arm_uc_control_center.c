...
static void ARM_UC_ControlCenter_Notification_Handler(void)
{
    if (arm_uccc_event_handler) {
        ARM_UC_PostCallback(arm_uccc_event_handler, ARM_UCCC_EVENT_MONITOR_SEND_DONE);
    }
}
    ...
    switch (request) {
        case ARM_UCCC_REQUEST_DOWNLOAD:
            if (arm_uccc_event_handler && arm_uc_download_token_armed) {
                ...
                ARM_UC_PostCallback(arm_uccc_event_handler,
    ...
    switch (request) {
        case ARM_UCCC_REQUEST_DOWNLOAD:
            if (arm_uccc_event_handler && arm_uc_download_token_armed) {
                ...
                ARM_UC_PostCallback(arm_uccc_event_handler, arm_uccc_event);
...
{
    ARM_UC_PostCallback(arm_uccc_override_task, 0);
}
...
arm_uc_error_t ARM_UC_ControlCenter_ReportState(arm_uc_monitor_state_t state)
{
    ...
    if (arm_uc_monitor_struct) {
        ...
        {
            UC_CONT_ERR_MSG("ARM_UC_ControlCenter_ReportState no monitor");
        }
...
arm_uc_error_t ARM_UC_ControlCenter_ReportUpdateResult(arm_uc_monitor_result_t updateResult)
{
    ...
    {
        UC_CONT_ERR_MSG("ARM_UC_ControlCenter_ReportUpdateResult no monitor");
    }
#if defined(TEST_USING_LOCAL_SOURCES) && TEST_USING_LOCAL_SOURCES == 1
    ARM_UC_ControlCenter_Notification_Handler();
#endif
...
arm_uc_error_t ARM_UC_ControlCenter_ReportName(arm_uc_buffer_t *name)
{
    ...
    {
        UC_CONT_ERR_MSG("ARM_UC_ControlCenter_ReportName no monitor");
    }
...
arm_uc_error_t ARM_UC_ControlCenter_ReportVersion(uint64_t version)
{
    ...
    {
        UC_CONT_ERR_MSG("ARM_UC_ControlCenter_ReportVersion no monitor");
    }
...
arm_uc_error_t ARM_UC_ControlCenter_ReportBootloaderHash(arm_uc_buffer_t *hash)
{
    ...
    {
        UC_CONT_ERR_MSG("ARM_UC_ControlCenter_ReportBootloaderHash no monitor");
    }
...
arm_uc_error_t ARM_UC_ControlCenter_ReportOEMBootloaderHash(arm_uc_buffer_t *hash)
{
    ...
    {
        UC_CONT_ERR_MSG("ARM_UC_ControlCenter_ReportOEMBootloaderHash no monitor");
    }
