...
// optimized verrsion without eventifying
static arm_uc_mdHandle_t debug_mdHandle = { 0 };

#ifdef RUN_HASH_IN_SINGLE_CALL
// todo dirty tricks, this is using syncronous single function to check content of
// downloaded image candidate against hash from the manifest
extern arm_uc_error_t arm_uc_pal_flashiap_get_slot_addr_size(uint32_t slot_id,
                                                             uint32_t *slot_addr,
                                                             uint32_t *slot_size);

extern int32_t arm_uc_flashiap_read(uint8_t *buffer,
                             uint32_t address,
                             uint32_t size);
extern uint32_t arm_uc_flashiap_get_page_size(void);

#include "../../pal-flashiap/update-client-pal-flashiap/arm_uc_pal_flashiap_implementation.h"
#define HEADER_SIZE (112)

static void arm_uc_internal_process_hash(void)
{
    UC_FIRM_ERR_MSG("arm_uc_internal_process_hash single call");
    ...
    UC_FIRM_TRACE("ARM_UC_cryptoHashSetup %u", result);
    result = arm_uc_pal_flashiap_get_slot_addr_size(package_configuration->package_id,
                                                                   &slot_addr,
                                                                   &slot_size);
    UC_FIRM_TRACE("arm_uc_pal_flashiap_get_slot_addr_size %u", result.code);
    uint32_t page_size = arm_uc_flashiap_get_page_size();
    uint32_t hdr_size = arm_uc_pal_flashiap_round_up_to_page_size(HEADER_SIZE);
    uint32_t physical_address = slot_addr + hdr_size;

    UC_FIRM_TRACE("slot addr:%p slot size:%u package size: %u, physical_address %p",slot_addr, slot_size, package_configuration->package_size, physical_address );
    int dataLeft = 0;
    for(int dataRead = 0; dataRead<package_configuration->package_size;)
    {
        dataLeft = package_configuration->package_size-dataRead;
        int size = MIN( dataLeft, front_buffer->size_max);

        // todo check result codes
        result.code = arm_uc_flashiap_read(front_buffer->ptr, physical_address+dataRead,size);
        UC_FIRM_TRACE("arm_uc_flashiap_read %u", result.code);
        dataRead+=size;
        front_buffer->size = size;

        UC_FIRM_TRACE("arm_uc_flashiap_read physical_address %p, size %u ", physical_address+dataRead, size );

        result = ARM_UC_cryptoHashUpdate(&mdHandle, front_buffer);
        UC_FIRM_TRACE("ARM_UC_cryptoHashUpdate %u", result.code);

#if UCFM_DEBUG_OUTPUT
        ARM_UC_cryptoHashSetup(&debug_mdHandle, mdtype);
        uint8_t debug_hash_output_ptr[2 * UCFM_MAX_BLOCK_SIZE];
        arm_uc_buffer_t debug_hash_buffer = {
            .size_max = sizeof(debug_hash_output_ptr),
            .size = 0,
            .ptr = debug_hash_output_ptr
        };
         ARM_UC_cryptoHashUpdate(&debug_mdHandle, front_buffer);
         ARM_UC_cryptoHashFinish(&debug_mdHandle, &debug_hash_buffer);
         UC_FIRM_TRACE("debug hash_buffer: %s", mbed_trace_array(debug_hash_buffer.ptr, ARM_UC_SHA256_SIZE));
#endif
    }
    ...
    result = ARM_UC_cryptoHashFinish(&mdHandle, &hash_buffer);
    UC_FIRM_TRACE("ARM_UC_cryptoHashFinish %u", result.code);

    int diff = memcmp(hash_buffer.ptr,
                      package_hash,
                      ARM_UC_SHA256_SIZE);
#if UCFM_DEBUG_OUTPUT
    UC_FIRM_ERR_MSG("own hash_buffer: %s", mbed_trace_array(hash_buffer.ptr, ARM_UC_SHA256_SIZE));
    UC_FIRM_ERR_MSG("package hash_buffer: %s", mbed_trace_array(package_hash, ARM_UC_SHA256_SIZE));
#endif

    if (diff == 0) {
        UC_FIRM_TRACE("UCFM_EVENT_FINALIZE_DONE hash match");
...
static void arm_uc_internal_process_hash(void)
{
...
#if UCFM_DEBUG_OUTPUT
        UC_FIRM_TRACE("double buffering: %p %" PRIX32 "\r\n", back_buffer, back_buffer->size_max);
#endif
    ...
    if (status.error == ERR_NONE) {
        ...
        arm_uc_error_t status2 = ARM_UC_cryptoHashUpdate(&mdHandle, front_buffer);
        UC_FIRM_TRACE("ARM_UC_cryptoHashUpdate: %u", status2);
        if(status2.code != 0)
        {
            UC_FIRM_TRACE("ARM_UC_cryptoHashUpdate non zero: %u", status2);
        }
        ...
        if (needs_more_data) {
            ...
            if (!double_buffering) {
#if UCFM_DEBUG_OUTPUT
                UC_FIRM_TRACE("single buffering: %p\r\n", front_buffer);
#endif
                // Indicate read size
                uint32_t bytes_remaining = package_configuration->package_size - package_offset;
                back_buffer->size = (bytes_remaining > back_buffer->size_max) ?
            ...
            // finalize hash calculation
            uint8_t hash_output_ptr[2 * UCFM_MAX_BLOCK_SIZE];
            memset(hash_output_ptr,0,2 * UCFM_MAX_BLOCK_SIZE);
            arm_uc_buffer_t hash_buffer = {
                .size_max = sizeof(hash_output_ptr),
                .size = 0,
                .ptr = hash_output_ptr
            ...
            arm_uc_error_t status3;
            status3 =  ARM_UC_cryptoHashFinish(&mdHandle, &hash_buffer);
            UC_FIRM_TRACE("ARM_UC_cryptoHashFinish: %u", status3);
            ...
            if (hash_buffer.size == ARM_UC_SHA256_SIZE) {
                ...
                if (diff == 0) {
                    ...
                    UC_FIRM_TRACE("Valid image hash, signalling success");
#if UCFM_DEBUG_OUTPUT
                    UC_FIRM_TRACE("hash_buffer: %s", mbed_trace_array(hash_buffer.ptr, ARM_UC_SHA256_SIZE));
                    UC_FIRM_TRACE("hash_buffer: %s", mbed_trace_array(package_hash, ARM_UC_SHA256_SIZE));
#endif
                    arm_uc_signal_ucfm_handler(UCFM_EVENT_FINALIZE_DONE);
                    status.code = ERR_NONE;
                } else {
                    // use specific event for "invalid hash"
                    UC_FIRM_ERR_MSG("Invalid hash check");
                    UC_FIRM_ERR_MSG("hash_buffer: %s", mbed_trace_array(hash_buffer.ptr, ARM_UC_SHA256_SIZE));
                    UC_FIRM_ERR_MSG("hash_buffer: %s", mbed_trace_array(package_hash, ARM_UC_SHA256_SIZE));
                    status.code = AUTH_ERR_INSTALL_REJECTED;
                    error_event = UCFM_EVENT_FINALIZE_INVALID_HASH_ERROR;
                }
                // clear local hash
                memset(package_hash, 0, ARM_UC_SHA256_SIZE);
    ...
    {
        UC_FIRM_ERR_MSG("error from read");
    }

    // signal error if status is not clean
    if (status.code != ERR_NONE) {
        UC_FIRM_TRACE("UCFM_EVENT_FINALIZE_ERROR %u", status.code);
        arm_uc_signal_ucfm_handler(error_event);
    }
}
#endif // RUN_HASH_IN_SINGLE_CALL
/******************************************************************************/
...
static void event_handler_finalize(void)
{
...
#ifndef RUN_HASH_IN_SINGLE_CALL
    ...
    if (result.error == ERR_NONE) {
        ...
        result = ARM_UCP_Read(package_configuration->package_id,
                              package_offset,
                              front_buffer);
        UC_FIRM_TRACE("event_handler_finalize front: %p size %u", front_buffer->ptr, front_buffer->size);
    }
...
{
#if UCFM_DEBUG_OUTPUT
    UC_FIRM_TRACE("event_handler_read: %" PRIX32 "\r\n", front_buffer->size);
#endif
...
static void ARM_UCFM_PALEventHandler(uintptr_t event)
{
    ...
    ARM_UC_PostCallback(arm_uc_internal_event_handler, event);
}
...
static arm_uc_error_t ARM_UCFM_Write(const arm_uc_buffer_t *fragment)
{
    ...
    if (!fragment || fragment->size_max == 0 || fragment->size > fragment->size_max || !fragment->ptr) {
        ...
        arm_uc_buffer_t debug_hash_buffer = {
         ...
         ARM_UC_cryptoHashUpdate(&debug_mdHandle, (arm_uc_buffer_t *)fragment);
         ARM_UC_cryptoHashFinish(&debug_mdHandle, &debug_hash_buffer);
         UC_FIRM_ERR_MSG("fragmentwrite offset: %u hash_buffer: %s", package_offset, mbed_trace_array(debug_hash_buffer.ptr, ARM_UC_SHA256_SIZE));
...
static arm_uc_error_t ARM_UCFM_Finalize(arm_uc_buffer_t *front, arm_uc_buffer_t *back)
{
    ...
    if (!ready_to_receive) {
        ...
        UC_FIRM_TRACE("setting front_buffer %p back_buffer %p", front_buffer, back_buffer);
        /* flush to PAL */
