...
{
    /* A zero event means no event - do not signal */
    if (arm_uc_pal_external_callback && event) {
        if (from_thread) {
            /* Run given callback in the update client's thread */
            ARM_UC_PostCallback(arm_uc_pal_external_callback, event);
        } else {
...
#else
        uint8_t temp_buffer[ARM_UC_INTERNAL_HEADER_SIZE_V2] = { 0 };
#endif
        arm_uc_buffer_t buffer = {
