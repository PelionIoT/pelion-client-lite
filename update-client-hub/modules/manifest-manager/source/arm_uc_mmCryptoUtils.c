                                        ...
                                        const arm_uc_buffer_t *sig)
{
    ...
    int rc = mbedtls_x509_crt_parse_der_nocopy(&crt, ca->ptr, ca->size);
    if (rc == MBEDTLS_ERR_X509_ALLOC_FAILED) {
        err.code = ERR_OUT_OF_MEMORY;
    } else if (rc < 0) {
        UC_MMGR_ERR_MSG("mbedtls_x509_crt_parse_der_nocopy returned error "PRIu32,  rc);
                                  ...
                                  const arm_uc_buffer_t *fingerprint)
{
    ...
    ARM_UC_PostCallback(ARM_UC_mmVerifySignatureEntry, event);
}
#endif /* ARM_UC_FEATURE_MANIFEST_PUBKEY */
/**
                                          ...
                                          uint32_t sigIndex)
{
    ...
    if (err.error == 0 && ctx) {
        ...
        ARM_UC_PostCallback(ARM_UC_mmVerifySignatureEntry, ARM_UC_MM_EVENT_BEGIN);
    }
    UC_MMGR_TRACE("%s %c%c:%hu (%s)\n", __PRETTY_FUNCTION__,
                                           ...
                                           uint32_t sigIndex)
{
    ...
    if (err.error == ERR_NONE && ctx) {
        ...
        int rc = ARM_UC_PostCallback(ARM_UC_mmVerifySignatureEntry, ARM_UC_MM_EVENT_BEGIN);
        if (!rc) {
