...
#include <string.h>
#define TRACE_GROUP "UT"
#include "mbed_trace.h"
void printDetails(arm_uc_firmware_details_t *details);
...
{
    tr_info("arm_uc_create_internal_header_v2");
    printDetails((arm_uc_firmware_details_t *)input);
    arm_uc_error_t result = { .code = ERR_INVALID_PARAMETER };
...
void printDetails(arm_uc_firmware_details_t *details)
{
    tr_info("version:%llu size %u", details->version, details->size);
    tr_info("hash:%s", tr_array(details->hash, ARM_UC_SHA256_SIZE));
}
...
arm_uc_error_t arm_uc_create_external_header_v2(const arm_uc_firmware_details_t *input,
                                                arm_uc_buffer_t *output)
{
    tr_error("arm_uc_create_external_header_v2");
    arm_uc_error_t result = { .code = ERR_INVALID_PARAMETER };
