...
static void ARM_UC_DeltaPaal_PALEventHandler(uintptr_t event)
{
    ...
    ARM_UC_PostCallback(arm_uc_deltapaal_internal_event_handler, event);
}
                                            ...
                                            const arm_uc_buffer_t *buffer)
{
    ...
    if (!update_payload_full) {
        ...
        if ( ARM_UC_PostCallback(ARM_UC_DeltaPaal_AsyncWrite_Handler, ARM_UC_PAAL_EVENT_WRITE_DONE) ) {
            result.code = ERR_NONE;
        }
