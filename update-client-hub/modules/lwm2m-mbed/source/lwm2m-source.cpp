                                                       ...
                                                       uint32_t offset)
{
    ...
    if ((buffer != NULL) &&
        ...
        if (length > buffer->size_max) {
            UC_SRCE_ERR_MSG("manifest too large to get buffer");
            ARM_UC_SET_ERROR(result, ERR_OUT_OF_MEMORY);
    ...
    if(result.code == ERR_NONE)
    {
        ...
        if (ARM_UCS_EventHandler) {
            ARM_UC_PostCallback(ARM_UCS_EventHandler, EVENT_MANIFEST);
        }
    }
...
bool ARM_UCS_LWM2M_SOURCE_manifast_received(const uint8_t *buffer, uint16_t length)
{
    ...
    // tapani todo here just call ARM_UCS_LWM2M_SOURCE_GetManifestDefault with buffer no need to create pointless temporary buffer to transfer via events
    arm_ucs_manifest_buffer = new uint8_t[length];  // tapani todo no runtime allocation was supposed to be used
    ...
    if (ARM_UCS_EventHandler) {
        ARM_UC_PostCallback(ARM_UCS_EventHandler, event_code);
    }
                                                        ...
                                                        uint32_t offset)
{
    ...
    if (uri &&
        ...
        // tapani todo check if arm_net_address_get etc ready made coud be used
        buffer->size = 0;
                                  ...
                                  void *context)
{
    ...
    if (output_buffer &&
        ...
        if (ARM_UCS_EventHandler) {
            ARM_UC_PostCallback(ARM_UCS_EventHandler, EVENT_FIRMWARE);
        }
    } else {
        /* Signal hub an error occurred. */
        if (ARM_UCS_EventHandler) {
            ARM_UC_PostCallback(ARM_UCS_EventHandler, EVENT_ERROR);
        }
    }
...
#ifdef MBED_CONF_MBED_CLIENT_ENABLE_CPP_API  // m2m interface is cpp api only
/**
...
arm_uc_error_t ARM_UCS_LWM2M_SOURCE_SetM2MInterface(M2MInterface *interface)
{
    ...
    (defined(ARM_UC_PROFILE_NANO_CLIENT) && (ARM_UC_PROFILE_NANO_CLIENT == 1))
