...
bool firmware_update_initialize(registry_t *registry)
{
    ...
    if (REGISTRY_STATUS_OK != registry_set_value_int(registry, &path, 0) ||
        REGISTRY_STATUS_OK != registry_set_auto_observable_parameter(registry, &path, true) ||
        REGISTRY_STATUS_OK != registry_set_resource_value_to_reg_msg(registry, &path, true)) {
...

{
    ...
    if (!value->generic_value.data.opaque_data->size) {
        ...
        send_final_response(path, registry->notifier->endpoint, token->token, token->token_size, COAP_MSG_CODE_RESPONSE_BAD_REQUEST, false);
        return REGISTRY_STATUS_OK;
    }
    ...
    if (ARM_UCS_LWM2M_SOURCE_manifast_received(value->generic_value.data.opaque_data->data, value->generic_value.data.opaque_data->size)) {
        send_final_response(path, registry->notifier->endpoint, token->token, token->token_size, COAP_MSG_CODE_RESPONSE_CHANGED, false);
    } else {
        send_final_response(path, registry->notifier->endpoint, token->token, token->token_size, COAP_MSG_CODE_RESPONSE_INTERNAL_SERVER_ERROR, false);
    }
...
bool firmware_update_send_pkg_version(registry_t *registry, uint64_t version)
{
    ...
    return (REGISTRY_STATUS_OK == registry_set_value_int(registry, &path, version));
}
