...
static arm_uc_notification_handler_t notificationHandler = NULL;
 ...
 */
arm_uc_error_t ARM_UC_HUB_Initialize()
{
 ...
 */
arm_uc_error_t ARM_UC_HUB_AddNotificationHandler(arm_uc_notification_handler_t handler)
{
...
bool ARM_UC_IsValidState(arm_uc_update_state_t an_update_state)
{
    ...
    if (!is_valid) {
        UC_ERROR_ERR_MSG("Invalid UC HUB reported state");
    }
    return is_valid;
}
...
static arm_event_storage_t storage_pool[ARM_UC_EVENT_POOL_SIZE] = {0};

static arm_event_storage_t *alloc_storage(void) {
    for (uint32_t i = 0; i < ARM_UC_EVENT_POOL_SIZE; i++) {
        if (storage_pool[i].data.data_ptr == NULL) {
            return &storage_pool[i];
        }
    }
...
bool ARM_UC_PostCallback(arm_uc_no_context_callback_t callback, uintptr_t parameter)
{
    ...
    arm_event_storage_t *storage = alloc_storage();

    UC_SDLR_TRACE("%s Scheduling %p(%lu)", __PRETTY_FUNCTION__, callback, parameter);
    return notificationHandler(storage, (void *)callback, parameter);
}


void ARM_UC_ProcessElement(arm_event_s* event)
{
    arm_uc_no_context_callback_t cb = (arm_uc_no_context_callback_t)(event->data_ptr);
    uintptr_t param = event->event_data;
    event->data_ptr = NULL;
    event->event_data = NULL;
    UC_SDLR_TRACE("%s Invoking %p(%lu)", __PRETTY_FUNCTION__, cb, param);
    cb(param);
}
#endif // ARM_UC_ENABLE
