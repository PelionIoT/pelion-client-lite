...
arm_uc_error_t ARM_UC_HUB_initSequence()
{
    ...
    */
    ...
    if (retval.error != ERR_NONE) {
        /* Just skip next steps if can't get active firmware details */
        UC_HUB_ERR_MSG("GetActiveFirmwareDetails fails with %u, skipping reporting to service", retval.error);
        memset( &arm_uc_active_details, 0, sizeof(arm_uc_active_details) );
    ...
    if (retval.error != ERR_NONE) {
        /* Just skip next steps if can't get installer details */
        UC_HUB_ERR_MSG("Active version reporting fails %u", retval.error);
        return ARM_UC_ERROR(ERR_NONE);
    }
    ...
    do {
        ...
        switch (arm_uc_hub_state) {
            ...
            case ARM_UC_HUB_STATE_NOTIFIED:
                ...
                manifest_buffer.size = 0;
                manifest_buffer.ptr = (uint8_t *) &fwinfo->manifestBuffer;

                retval = ARM_UC_SourceManager.GetManifest(&manifest_buffer, 0);
            ...
            case ARM_UC_HUB_STATE_MANIFEST_FETCHED:
                ...
                fwinfo->manifestSize = manifest_buffer.size;
            ...
            */
            case ARM_UC_HUB_STATE_SETUP_FIRMWARE: {
                ...
                memcpy(arm_uc_hub_firmware_details.hash,
                       fwinfo->hash.ptr,
                       ARM_UC_SHA256_SIZE);
                tr_debug("Copied image hash to firmware details");
#endif
                ...
                */
                if (fwinfo->size == 0) {
                    ...
                    if (retval.code !=ERR_NONE) {
                        ...
                        HANDLE_ERROR(retval, "GetFirmwareFragment failed ARM_UC_HUB_STATE_FETCH_FIRST_FRAGMENT")
                    }
                }
                break;
                ...
                */
                ...
                if (firmware_offset < fw_downloadSize) {
                    ...
                    if (retval.code != ERR_NONE) {
                        ...
                        ARM_UC_HUB_ErrorHandler(SOMA_ERR_NO_ROUTE_TO_SOURCE,
                                                ARM_UC_HUB_STATE_STORE_AND_DOWNLOAD);
                        HANDLE_ERROR(retval, "GetFirmwareFragment failed ARM_UC_HUB_STATE_STORE_AND_DOWNLOAD")
                    }
                } else {
