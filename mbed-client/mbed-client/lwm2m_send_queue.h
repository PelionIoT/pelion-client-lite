...
typedef struct send_queue_s {
    ...
    send_queue_sender_t last_sender; ///< Type of the last message sent, used for scheduling different types in turns.
                                     ///< Does not include `SEND_QUEUE_ENDPOINT` as it always gets sent first if requested.
                                     ///< We are not using the `SEND_QUEUE_ENDPOINT` with this field.
