...
*
*  This set checks and validates the compile-time options for the client library.
...
// Client Lite can only use LITE update client
#if defined(ARM_UC_PROFILE_MBED_CLIENT_LITE) && !(ARM_UC_PROFILE_MBED_CLIENT_LITE==1)
#error "Client Lite must use ARM_UC_PROFILE_MBED_CLIENT_LITE=1 setting."
#endif

#if defined(MBED_CLOUD_CLIENT_FOTA_ENABLE) && defined(MBED_CLOUD_CLIENT_SUPPORT_UPDATE)
#error "FOTA and Update client cannot be enabled together"
#endif
...
#error "MBED_CLOUD_CLIENT_FOTA_STORAGE_SIZE must be set"
#endif
