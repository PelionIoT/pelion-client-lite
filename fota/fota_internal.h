...
typedef struct {
    ...
    // All fields up until now are persistent
    // and should be kept in NVM for restore functionality.
    // Those this line are not.
    ...
    uint32_t fw_offset_in_storage;
    uint32_t storage_end_addr;
    uint8_t *metadata_start_buf;
    uint8_t *metadata_buf;
    uint32_t metadata_buf_size;
    uint32_t metadata_buf_offset;
    uint32_t metadata_start_size;
    uint32_t metadata_tags_size;
    uint32_t metadata_curr_addr;
