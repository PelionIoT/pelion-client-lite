    ...
    manifest_firmware_info_t *fw_info, const uint8_t *input_data)
{
    ...
    FOTA_MANIFEST_TRACE_DEBUG("Manifest:payload-version %" PRIu64, fw_info->version);
...
)
{
    ...
    tls_status = mbedtls_asn1_get_tag(
                     ...
                     MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (tls_status != 0) {
        FOTA_TRACE_ERROR("Error SignedResource tag %d", tls_status);
        return FOTA_STATUS_MANIFEST_MALFORMED;
    }
    ...
    tls_status = mbedtls_asn1_get_enumerated_value(&p, signed_resource_end, &manifest_format_version);
    if (tls_status != 0) {
        FOTA_TRACE_ERROR("Error reading SignedResource:version %d", tls_status);
        return FOTA_STATUS_MANIFEST_MALFORMED;
    }
    ...
    tls_status = mbedtls_asn1_get_tag(
                     ...
                     MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (tls_status != 0) {
        FOTA_TRACE_ERROR("Error reading SignedResource:manifest %d", tls_status);
        return FOTA_STATUS_MANIFEST_MALFORMED;
    }
    ...
    tls_status = mbedtls_asn1_get_tag(
                     ...
                     MBEDTLS_ASN1_OCTET_STRING);
    if (tls_status != 0) {
        FOTA_TRACE_ERROR("Error reading SignedResource:signature %d", tls_status);
        return FOTA_STATUS_MANIFEST_MALFORMED;
    }
